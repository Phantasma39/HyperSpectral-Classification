...
### Q: 训练时只用了有标签的像素吗？预测图是全图覆盖吗？
**是的，只训练有标签的像素。** 模型只对那 10,249 个标注像素进行训练和评估。预测图 `07_classification_map.png` 也只有这 10,249 个彩色点，大片黑色是未标注的背景区域，模型从未预测过它们。

标注像素本身就是聚集的（比如森林区域标注了 1,265 个像素，玉米地标注了几千个），所以预测图看起来是连成一片的色块——那是标注者的选择，不是模型补全的。

### Q: 一个数据集是一张图还是很多张图？
**一张完整的遥感影像。** Indian Pines 就是一张 145×145 像素的航拍照片，PaviaU 是 610×340，Houston 是 210×954。这和传统图像分类（几千张猫狗照片各一张）完全不同——高光谱是在一张大图上做语义分割。
